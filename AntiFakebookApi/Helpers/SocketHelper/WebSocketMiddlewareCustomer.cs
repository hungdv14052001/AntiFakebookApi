...
namespace AntiFakebookApi.Helpers.SocketHelper
{
    public class WebSocketMiddlewareCustomer
    {
        private readonly RequestDelegate _next;
        private WebSocketHandler _webSocketHandler { get; set; }

        public WebSocketMiddlewareCustomer(RequestDelegate next, WebSocketHandler webSocketHandler)
        {
            _next = next;
        ...
        private async Task HandleMessage(WebSocket socket, string message)
        {
            ...
            else if (clientMessage.IsTypeChat())
            {
                ...
                {
                    ServerMessage chatMessage = new ServerMessage(clientMessage, _webSocketHandler.GetAllUsers());
